			...
			<div class="side-bar col-md-3">
				<form action="{{ url('lessonList/search') }}" method="GET">
					...
					<div class="left-side">
						...
						<input type='text' class="form-control" name="lessonDate" id="lessonDate" value="{{ isset($request) ? $request->lessonDate : '' }}"/>
					</div>
					<!-- //開催日 -->
					<div class="left-side">
	...
	<script>
		...
		$(window).load(function () {
			...
			@if (isset($lineUserData))
				localStorage.setItem('displayName', '{!! $lineUserData->displayName !!}');
				localStorage.setItem('pictureUrl', '{!! $lineUserData->pictureUrl !!}');
			@endif
			@if (isset($user))
